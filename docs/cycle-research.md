...
运行 `./start.ps1`，选择股票，在顶部全局模型选择器中选择模型，打开“周期研究”并点击“生成周期研究”。该选择由 FinRobot、周期研究、UZI 和财报分析共用。切换标签或刷新会恢复当前任务或已落盘报告。旧报告明确保留原快照时间；重新生成会重新取快照。重启后只能恢复已落盘的成功报告。
...
历史实现阶段验证记录（非当前测试总数）：224 项离线后端测试通过，44 项页面测试通过；获授权的一次 DeepSeek `deepseek-v4-flash` 烟测通过，执行 32 步工具操作、完整阅读 6 个章节、生成 7 个维度，当前事实数组全部为空。当时烟测输出路径为 `target/data-verification/cycle-skill-smoke.json`，该忽略目录可能被清理，不保证文件仍在。真实行情加模型的完整研究在该记录中尚未验证：首次尝试在沙箱中因核心数据获取失败停止。2026-09-15 仅核对文档，未重跑该烟测。
