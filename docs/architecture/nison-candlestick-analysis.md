...
- 主 K 线源的最新交易日早于独立核验源时，聚合层使用更新的核验 K 线，并把分区标记为 `DEGRADED`、保留主源与替代源信息。
- 派生结果继承 K 线分区的状态、问题和来源信息。

## 分析顺序

技术页首先展示最近已收盘日线复盘（兼容保留 `previousSession` 字段），再展示原有周期汇聚与近期形态。
以 `Asia/Shanghai` 的 15:05 为收盘安全时间：此前排除当天日线，此后允许纳入当天完整日线；
始终排除未来日期。该模块在切换周/月线时仍使用日线。
休市、停牌或供应商滞后都可能使实际日期提前，页面展示原始日期与来源状态，
不把数据中最近日期宣称为经交易日历核实的上一市场交易日。

复盘包含 OHLC 比例图、实体与上下影线占振幅比例、前收涨跌、相对前 20 日均量、
此前 5 根日线趋势及前 20 根日线区间，并关联仅在该日结束的命名形态。
所有计算与确认截止复盘日，不使用之后的数据倒推确认；未命名时仍提供结构解读。
阴阳按开收价判断，与相对前收的涨跌分开。无振幅的比例、缺少前收的涨跌、
...
`methodology.knowledge` 附带随分析返回的书本方法笔记，详见 [书本方法知识库](book-knowledge.md)。当前知识库共 17 条尼森条目：14 条 `IMPLEMENTED` 会进入自动分析上下文，3 条 `REFERENCE_ONLY` 仅作为内部扩展参考，不代表系统已经能够自动识别对应形态。
