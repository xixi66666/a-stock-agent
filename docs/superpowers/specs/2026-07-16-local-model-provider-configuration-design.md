...
The OpenAI template uses `https://api.openai.com` and leaves the model identifier as an account-specific placeholder. The DeepSeek template uses `https://api.deepseek.com` and `deepseek-chat`. The Xiaomi MiMo template uses its documented OpenAI-compatible base URL `https://api.xiaomimimo.com/v1` and `mimo-v2.5-pro`.
